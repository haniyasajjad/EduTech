...
import database.AssessmentsinDB;
...
public class addAssignment {
    ...
    private void handleUploadAssignment() {
        ...
        if (assignmentName.isEmpty() || duration.isEmpty() || courseName.isEmpty()) {
            showAlert("Validation Error", "Please fill all assignment details before uploading.", Alert.AlertType.WARNING);
            return;
        }

        if (questionList.isEmpty()) {
            showAlert("Validation Error", "Please add at least one question before uploading the assignment.", Alert.AlertType.WARNING);
            return;
        }
        ...
        try {
			...
			boolean check = Admin.getInstance().addAssignment(assignmentName, duration, courseName, qslist);
			if(check) 
			{
				// Display confirmation alert
		        showAlert("Success", "Assignment uploaded successfully for course: " + courseName, Alert.AlertType.INFORMATION);
                AssessmentsinDB.saveAssignment(assignmentName,10,qslist);
