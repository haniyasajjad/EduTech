    ...
    private void loadStudents() {
        ...
        studentsTable.setItems(studentRows);
            ...
            private VBox createGroupCard(StudyGroup group) {
                ...
                try {
                    // Load the FXML file for the messaging screen
                    FXMLLoader loader = new FXMLLoader(getClass().getResource("/sda/GroupMessaging.fxml"));
                    Parent messagingRoot = loader.load(); // Loaded root for the new screen

                    // Print values to check if the groupId and studentId are correct
                    System.out.println(group.getId() + "  " + App.currentStudent.getId());

                    // Get the controller and set the groupId and studentId
                    GroupMessagingController controller = loader.getController();
                    controller.setGroupAndStudent(group.getId(), App.currentStudent.getId());

                    // Set the loaded root as the new scene root
                    App.setRoot("GroupMessaging");  // Set the scene with the loaded root
